...
use super::{
    HttpFieldName, HttpHeader, HttpMethod, HttpStatusCode, HttpTarget, HttpVersion,
    PartialHttpRequest,
};
...
impl HttpRequest<'_> {
    ...
    pub fn try_parse<'a>(
        partial_request: &PartialHttpRequest<'a>,
        request_bytes: &'a [u8],
    ) -> Processing<PartialHttpRequest<'a>, Result<HttpRequest<'a>, (io::Error, HttpStatusCode)>>
    {
        let mut partial_request = partial_request.clone();
        let word_delimiter = b" ";
        let line_delimiter = b"\r\n";
        let body_delimiter = b"\r\n\r\n";
        let bad_request = Processing::Finished(Err((
            io::ErrorKind::InvalidInput.into(),
            HttpStatusCode::BadRequest400,
        )));
        let not_implemented = Processing::Finished(Err((
            io::ErrorKind::InvalidInput.into(),
            HttpStatusCode::NotImplemented501,
        )));
        let version_not_supported = Processing::Finished(Err((
            io::ErrorKind::InvalidInput.into(),
            HttpStatusCode::HttpVersionNotSupported505,
        )));

        // Method
        if partial_request.request.method.is_none() {
            partial_request.request.method =
                match Self::find_until(&mut partial_request, request_bytes, word_delimiter) {
        ...
        if partial_request.request.target.is_none() {
            partial_request.request.target =
                match Self::find_until(&mut partial_request, request_bytes, word_delimiter) {
                    None => return Processing::InProgress(partial_request),
                    Some(before_delimiter) => match std::str::from_utf8(before_delimiter) {
                        Err(_) => return bad_request,
        ...
        if partial_request.request.version.is_none() {
            partial_request.request.version =
                match Self::find_until(&mut partial_request, request_bytes, line_delimiter) {
                    None => return Processing::InProgress(partial_request),
                    Some(before_delimiter) => match std::str::from_utf8(before_delimiter) {
                        Err(_) => return bad_request,
        ...
        if partial_request.request.header.is_none() {
            partial_request.request.header =
                match Self::find_until(&mut partial_request, request_bytes, body_delimiter) {
                    None => return Processing::InProgress(partial_request),
        ...
        if partial_request.request.body.is_none() {
            partial_request.request.body = match &mut partial_request.request.header {
                None => None,
                Some(header) => match header.0.entry(HttpFieldName::ContentLength.to_string()) {
                    hash_map::Entry::Vacant(_) => None,
                    hash_map::Entry::Occupied(entry) => match entry.get().parse::<usize>() {
                        Err(_) => return bad_request,
                        Ok(content_length) => {
                            if content_length == 0 {
                                None
            ...
            match host.rfind(domain_name) {
                ...
                Some(index) => {
                    ...
                    if subdomain.ends_with(subdomain_delimiter) {
                        return Some(
                            &subdomain[..(subdomain.len() - subdomain_delimiter.len_utf8())],
                        );
                    }
                    return None;
                }
    ...
    fn find_until<'a>(
        partial_request: &mut PartialHttpRequest,
        request_bytes: &'a [u8],
        delimiter: &[u8],
    ) -> Option<&'a [u8]> {
    ...
    mod try_parse {
        ...
        fn full_request() {
            ...
            if let Processing::Finished(Ok(request)) = result {
                ...
                panic!(
                    "Expected Processing::Finished(Ok(HttpRequest)), but got {:?}",
                    result
                );
            }
        }
        ...
        fn partial_up_until_method() {
            ...
            if let Processing::InProgress(partial_request) = result {
                ...
                panic!(
                    "Expected Processing::InProgress(PartialHttpRequest), but got {:?}",
                    result
                );
            }
        }
        ...
        fn invalid_method() {
            ...
            if let Processing::Finished(Err(err)) = result {
                ...
                panic!(
                    "Expected Processing::Finished(Err(Error, HttpStatusCode)), but got {:?}",
                    result
                );
            }
        }
